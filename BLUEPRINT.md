...
* Affixes, not only suffixes: tables draw from the bundle's whole **affix inventory** — overwhelmingly suffixes, plus the occasional **prefix token** (Turkish has a handful of lexicalized ones: *na-*, *gayri-*). Prefixes are blocked, picked, topped up, and held under exactly the same rules; the only difference is where they attach (§4). They appear at natural inventory frequency — no quota — and "suffix" in session mechanics always reads as "affix".
...
* One chance each: a suffix **picked**, **blocked**, or left unpicked at round close is out of circulation for the rest of the match. Blocks are a denial tool: a blocked suffix is non-selectable for the entire round and does not carry forward to later rounds. Boards still clear at every round's scoring (used → consumed, unused → discarded).
...
* Bluff & flag eligibility — skin in the game: the construction-window-open stack snapshot sets two gates for the round. A player with **fewer than 60 chips** may not submit an uydurum; a player with **fewer than 20 chips** may not flag. Word chips are the fastest way back into both pools. The check is frozen at window open (same freeze rule as the draft) and covers both windows. The client refuses to lock in a non-valid word for an underfunded bluffer and hides the flag control for an underfunded detective; if either intent arrives anyway, the server rejects it — the uydurum is scored as **no submission**, the flag is simply dropped. The 100-chip seed (§5) puts everyone well above both gates in round 1, so the very first round has full bluff/flag access. Priced-in ripples: stacks are public, so a player below the 60-chip bluff gate has a known-real word (flagging it is a guaranteed false flag; the 20-chip fee applies as usual), and the whole table knows who is out of the bluff and flag pools.
...
One number per player: a **stack**, seeded at **100 chips** at match start and floored at **0** — nobody goes negative. Bluffing requires **60+ chips** and flagging requires **20+ chips** at construction-window open; a player below the relevant gate is locked out of that action for the round (§4). Words **mint** new chips from the bank; gambles **move** chips between players; the biggest stack at match end wins. The scoreboard shows two lines per player — **word chips** and **gamble chips** — so building and gambling read as two visibly different games.
...
| Uydurum attempted below 60 chips | rejected server-side (§4): scored as no submission |
...
│       │   └── widgets/         # Brutalist UI (BrutalistTimer, AffixCard, PickedAffixBoard, BluffButton, ReadyButton, PokeNudge, WordOfDayCard)
...
* The server owns the phase clock: every phase window (15 s root draft, 15 s shared affix block, 15 s per pick turn, 15 s submission, 15 s flagging) opens and closes on server time, and late intents are rejected server-side. Client timers are display-only.
