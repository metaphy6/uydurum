...
* Round Clock: draft 15 s (skipped in the final round) + `players × (15 s owner block + 10 s shared pick)` + construction 15 s + flag window 15 s. Worst case at 6 players: **3 min 15 s per round** (**3 min** in the final round). An owner locking blocks and Ready unanimity (§7) can only shorten a round, never extend it.
...
### 3. Phase 2: The Affix Session (15 s Block + 10 s Pick per Owner Turn)

* The session: each round has **one turn per player** (4 players → 4 turns; 6 → 6), rotating in draft-priority order. Every turn belongs to one **turn owner** and consists of a private block window followed by a reveal and shared pick window.
* Private sets: before the session, every seat receives its own hidden set of **players + 10** affix instances (4 players → 14; 16 at the 6-player maximum). Fresh sets are sampled to be at least **90% different by affix label** from every other player's set. At supported table sizes, the pairwise overlap cap rounds down to one matching label; intentional matches are separate dealt instances. Blocked carryovers described below are exempt from this fresh-deal overlap check.
...
* Block window (15 seconds, owner only): only the turn owner can see their set. They may send **one block action** containing **1, 2, or 3 affix instance IDs**. Once the server accepts it, the selection is final, repeat or replacement actions are rejected, and the table reveals immediately. If the window expires without an action, the table reveals with no blocks — the neutral behavior used for an absent owner. No other player can block that set.
* Pick window (10 seconds, everyone): the owner's set is revealed to all; blocked affixes remain visible but are non-selectable. The public discard row is visible beneath it. Every player may act once, in secret: take one selectable affix from either source, drop one affix from their board, or pass. Takes are hidden and **non-exclusive**, so several players may receive the same selected affix without a latency contest.
* Turn close and discards: after blind takes resolve, a source instance selected by anyone leaves its table or the discard row and joins the picked board; every player who selected it receives that affix. Drops then enter the public row without owner names and become selectable from the **next** table reveal, never midway through the window in which they were dropped. An unpicked discard stays public across turns and rounds until taken or until the match ends.
* One chance each, blocks carry forward: an unblocked table instance that nobody takes expires at that turn's close. A blocked instance instead replaces a fresh slot on a later owner's table and arrives unblocked; a later owner may block it again. It keeps returning until taken while unblocked or until the match ends. A deliberately repeated affix label in another player's fresh set is an independent instance, not the same carried token. Player boards still clear after every round's scoring (used → consumed, unused → discarded).
* Public information: every player's drafted root is visible all match (§2); the revealed table, its owner's blocks, the no-names discard row, and the no-names picked board are public. **Who took or dropped each affix and who currently holds it remain hidden.** The picked board lists each affix label once without ownership or duplicate counts.
* The minimum: every board must hold **at least 3 affixes** when construction begins. Over the session's last three owner turns, the server tops up anyone behind pace — 1 by the third-last, 2 by the second-last, 3 by the last. A top-up uses a selectable instance from the current revealed table and resolves like a hidden non-exclusive take, so the block rule is never bypassed and a legal source is guaranteed by the table size. A player who never acts is dealt exactly one per turn across those last three.
* No cap: one take per owner turn means a board can never exceed `players` affixes. Every affix that cannot fit the submitted word forfeits the clean-sweep bonus (§5), so hoarding still has an opportunity cost.
* Rationale: mostly distinct private sets prevent one globally bad deal from constraining everyone. The owner gets a meaningful denial decision, while each reveal gives the whole table the same hidden, non-exclusive action. Session length remains deterministic: `players × 25 s` (**2 min 30 s** at six players), and early block confirmation plus Ready can shorten it.

### 4. Phase 3: The Showdown & Bluffing Mechanic

* Structure — three server-owned steps: a **15-second construction & submission window** (compose from your root + board affixes and lock in one final word; no submission = no word chips), a **simultaneous reveal** of all words, then a **15-second blind flag window**. Flags stay hidden until the window closes — no bandwagoning, no fastest-tap race — and all resolve together at close. Each player may flag **at most one** word per round, **never their own**, and **only with at least 20 chips**: self-flags and underfunded flags are rejected server-side, so a bluffer cannot hedge their own submission and a nearly-broke player cannot recklessly police the table.
...
* The Deception Loop:
  ...
  * Caught bluff: the bluffer pays only that bluff's **20/40/60 stake**, split among all correct flaggers. Each receives `floor(stake / catchers)`; any remaining whole chips go one each to catchers in the round's rotating draft-priority order. The stake is a total liability, not an amount paid to every catcher.
...
One number per player: a **stack**, seeded at **100 chips** at match start and floored at **0** — nobody goes negative. Bluffing requires enough to cover the player's next **20/40/60** stake and flagging requires **20+ chips** at construction-window open; a player below the relevant gate is locked out of that action for the round (§4). Words **mint** new chips from the bank; gambles **move** chips between players; the biggest stack at match end wins. The scoreboard shows two lines per player — **word chips** and **gamble chips** — so building and gambling read as two visibly different games.
...
No tie-breaking cascade: the longest-word bonus is shared on ties, so nothing downstream needs an order — the old "fewer affixes" and "earliest submission" rules are gone (no server-arrival races anywhere). A round with no valid word simply mints no word chips and awards no longest-word bonus.
...
* Seats persist: a dropped-out player's seat is never removed — the system auto-plays it exactly like a fully passive player (no ranks → a root is still dealt; its private-table turn applies no blocks and reveals immediately; every pick window is a pass; last-three-turn top-ups still fill the board to three; no submission → no word chips minted). Root-pool consumption and the round count are both fixed at match start, so departures never shrink either — the `players × rounds` pool arithmetic holds regardless of who is present.
...
* Ready: in a private block window, the owner confirms one final block selection and the table reveals immediately; nobody else has a Ready action. In every shared pick window and the construction window, a locked action or pass enables Ready, and unanimity closes the window early. **Auto-played seats submit no blocks, pass, and count as always ready**, so a match running at the 2-connected minimum (§6) still fast-forwards. The root draft and flag window always run their full 15 seconds.
* Poke: once per wait window, any player may poke the block owner or one player who has not readied in a shared window. The target's device gives a **light haptic buzz** and a brief screen shake — pressure, not punishment: pokes are anonymous, have no score effect, and the once-per-window cap is enforced server-side.
...
* Storyboard (7 beats, 4–6 s each):
  ...
  3. Block — one player's private set appears only to them; they lock 1–3 affixes.
  4. Reveal & pick — that set flips public, blocks gray out, and hidden takes resolve beside the public discard row.
...
* The server owns the phase clock: every phase window (15 s root draft, 15 s private owner block + 10 s shared pick per owner turn, 15 s submission, 15 s flagging) opens and closes on server time, and late intents are rejected server-side. Client timers are display-only.
...
* Protocol: One persistent WebSocket per client. Clients send **intents** (`rank_roots`, `lock_affix_blocks`, `take_affix`, `drop_affix`, `pass_affix_turn`, `submit_word`, `flag_bluff`); the server responds with **state events** (`phase_started`, `roots_resolved`, `affix_table_revealed`, `affix_turn_resolved`, `round_scored`) broadcast to the lobby. `lock_affix_blocks` carries 1–3 instance IDs and is accepted at most once for the active owner turn. Messages are versioned JSON with sequence numbers for ordered replay.
* Fair arbitration: the root draft, each shared affix-pick window, and the bluff-flag window are blind-simultaneous — collected privately and resolved at window close — so no game-deciding event is a fastest-tap race. Owner blocks are private until their single irreversible action reveals the table; the server is the referee for every deadline and duplicate intent.
...
* Task 2: Build the server-side phase state machine: server-owned phase timers (15 s root draft, 15 s private owner block + 10 s shared pick per turn, 15 s submission, 15 s blind flag window), blind root-pick resolution with catch-up tie-breaks, the `players × rounds` root pool with random final-round deal, mostly distinct private affix sets, irreversible 1–3-instance owner blocks, hidden non-exclusive takes, blocked-affix carry-forward, the public discard row and picked board, last-three-turn top-ups, showdown submission, progressive 20/40/60 bluff stakes, blind flag resolution, and deterministic settlement.
* Task 3: Build the corresponding Flutter screens and phase state management against the live protocol.
* Testing Criteria: A scripted 6-bot integration test plays full matches against the Compose stack, including forced mid-round disconnects/reconnects, with no state corruption; contested root picks resolve to exactly one winner; fresh private sets satisfy the overlap cap; a second block action is rejected; a dropped affix first becomes selectable on the next reveal; and caught first/second/later bluffs transfer 20/40/60 chips without charging unchallenged bluffers.
